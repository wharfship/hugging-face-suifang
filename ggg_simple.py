...
field_attempts = {}
chat_history = []   # simple 版也统一使用当前会话里的 chat_history
...
def export_tracker_data():
    ...
    keep_columns = ["field", "value", "completion", "status"]
    if not df.empty:
...
def process_user_input(user_message, chat_history):
    ...
    parse_output = f"流程状态: {result['status']}, 完整度: {result['completion']}, 置信度: {result['confidence']}"
...
with gr.Blocks(title="AI医疗随访系统") as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=3):
            # 去掉 type='messages'，兼容你当前环境里的 gradio 版本。
