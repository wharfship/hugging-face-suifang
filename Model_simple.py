...
def parse_answer(field, answer, description, history):
   ...
   status说明：
   pending，skip，ambiguous三种状态只有当其对应confidence>0.75时才可将status设为该状态，否则设为其他状态。
    - success: 患者已经基本回答了全部问题，无需再补充信息
    - partial_success: 患者的当前回答：{answer}，在上一次回答的基础上，或对提问进行了正常的回答，或又进一步回答了部分问题，但当前问题还需要继续提问以补充信息
    - ambiguous: 患者当前回答：{answer}，为空白，或者达非所问，或对问题的回答比较模糊，或不符常人认知。
    - skip: 患者明确表示因为不愿意或因为忘记而无法提供此信息。比如患者回答：："忘记了，不记得"或者“我不愿意回答”
    - pending: 患者无法回答该问题，但患者明确承诺会上传相关医疗图片或其他数据体检相关数据（如化验单）
   ...
   - 若问"您有哪些慢性病？"包含多个独立部分(高血压、糖尿病等)，但只询问了部分，患者回答"高血压记不清了"，应标记为"partial_success"继续询问其他疾病
    ...
    try:  # 以下有try和except代码的解释。
        ...
        print(f"解析结果：{re_result['field_value']}  置信度：{re_result['confidence']}")
        return re_result
